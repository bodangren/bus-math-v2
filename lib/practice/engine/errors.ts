...
function buildTranspositionScenario(seed: number, options: Required<TrialBalanceErrorGenerationConfig>, rowIndex: number) {
  ...
  return buildScenarioFromOutcome({
    ...
    narrative: `One amount was transposed from ${formatAmount(correctAmount)} to ${formatAmount(erroneousAmount)}.`,
    whatHappened: 'A digit swap changed a single amount.',
...
function buildSlideScenario(seed: number, options: Required<TrialBalanceErrorGenerationConfig>, rowIndex: number) {
  ...
  return buildScenarioFromOutcome({
    ...
    narrative: `One amount slid from ${formatAmount(correctAmount)} to ${formatAmount(erroneousAmount)}.`,
    whatHappened: 'A digit slipped out of a single amount.',
