...
| 2026-03-13 | curriculum_authoring_publish_pipeline | Unit 1 authored spreadsheet and teacher-submission contracts still require publish-time normalization instead of passing the current activity/section validators directly | Medium | Open | Handle this in the Unit 1 canonicalization track instead of widening runtime schema support further. |
| 2026-03-13 | teacher_monitoring_core | Teacher lesson monitoring currently adapts published Convex lesson data into the legacy `TeacherLessonPlan` validator shape instead of a dedicated published-curriculum presentation contract | Low | Open | Revisit this when Unit 1 canonicalization tightens the teacher-facing lesson-plan presentation surface. |
