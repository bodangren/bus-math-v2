...
| Middleware / Access | PASS | `proxy.ts`, `docs/security-api-route-matrix.md`, `lib/api/test-route-guard.ts`, `__tests__/proxy.test.ts` | Proxy now allowlists debug endpoints `/api/test-db` and `/api/test-supabase`, while route-level env/key guard remains enforced and private API routes stay deny-by-default. |
...
1. `GAP-01` Proxy/public debug-route mismatch - **Completed** (`0c71565`)
...
4. `GAP-04` Seed runbook drift - **Completed** (`1e3537f`)
