...
| Lesson Page (error states) | Student Dashboard | Return path | `studentDashboardPath()` | ❌ Uses hardcoded "/student/dashboard" |
| Lesson Page (completed) | Student Dashboard | Return path | `studentDashboardPath()` | ❌ Uses hardcoded "/student/dashboard" |
| Student Dashboard Unit Cards | Lesson | Full navigation | `studentLessonPath(slug)` | ❌ Not audited yet |
...
|---------|-----------------|---------------|
| Student Dashboard | None (top level) | N/Audi |
| Lesson Page | [Student Dashboard → Lesson] | Should add breadcrumb navigation component |
| Lesson (Phase View) | [Student Dashboard → Lesson → Phase N] | Should add breadcrumb navigation component |
...
| Teacher Student Detail | [Teacher Dashboard → Student Details] | ❌ Uses "Back to dashboard" link to `/teacher` (should be `/teacher/dashboard`) |
...
2. **Consider adding breadcrumb components:**
   - Student lesson pages should show [Dashboard → Lesson] breadcrumb trail
   - Student lesson phase views should show [Dashboard → Lesson → Phase N] breadcrumb trail
   - Teacher student detail page should show [Dashboard → Student] breadcrumb trail

3. **Define canonical breadcrumb component (if needed):**
   - Shared component that renders: Home > Dashboard > [Optional Context] > Current Page
   - Uses navigation helpers for links
   - Respects role for dashboard destination

## Notes

- UserMenu role-aware dashboard link: ✅ Complete (Phase 1 Task 1)
- No student unit surface by design: unit anchors scroll to dashboard sections
- Lesson page error states already have "Return to Dashboard" links (just need to use helpers)
