...
- Next.js error.tsx files must be client components ('use client') receiving Error & { digest?: string } and reset: () => void props. Place at route segment level (not layout level) to catch page-level errors without takeover of sibling routes.
